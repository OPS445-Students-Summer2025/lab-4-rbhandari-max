...
def join_sets(s1, s2):
    ...
    return s1 | s2  # union of s1 and s2

def match_sets(s1, s2):
    # match_sets will return a set that contains all values found in both s1 and s2
    return s1 & s2  # intersection of s1 and s2

def diff_sets(s1, s2):
    # diff_sets will return a set that contains all different values which are not shared between the se>
    return s1 ^ s2  # symmetric difference of s1 and s2
